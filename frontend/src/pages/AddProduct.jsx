...
export default function AddProduct(){
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            setError("")
            try{
                const [shopRes, vendorRes, categoriesRes] = await Promise.all([
                    API.get("/shop/my"),
                    API.get("/user/vendor"),
                    getCategories()
                ])
                ...
                if(!cancelled) setError("Failed to load add product page.")
            }finally{
    ...
    const handleSubmit = async (event) => {
        ...
        try{
            await API.post(`/shop/products/${encodeURIComponent(form.categoryName)}`, {
                name: form.name,
        ...
        <div className="mp-shell">
            ...
            <main className="mp-container">
                <nav className="pd-breadcrumb" aria-label="Breadcrumb" style={{ paddingTop: 12 }}>
                    ...
                    <span className="pd-sep" aria-hidden="true">/</span>
                    <span className="pd-crumb pd-crumb-current">Add product</span>
                </nav>
                ...
                {loading ? (
                    ...
                    <section className="pd-card">
                        <h1 className="pd-title">Add product</h1>
                        <p className="pd-muted">You need an approved vendor account and a shop first.</p>
                    </section>
                ) : (
                    <section className="pd-card">
                        <div className="ud-head">
                            <h1 className="pd-title" style={{ fontSize: 22 }}>Add product</h1>
                            <div className="pd-muted">Create a new product for {shop.name}.</div>
                        </div>
    ...
    {flash ? <div className="pd-flash" style={flash.toLowerCase().includes("failed") || flash.toLowerCase().includes("please") ? { background: "#fff2f2", borderColor: "#f1c1c1", color: "#7a0b0b", marginBottom: 10 } : { marginBottom: 10 }}>{flash}</div> : null}
                        ...
                        <form className="ud-form" onSubmit={handleSubmit}>
                            ...
                            <div className="pd-actions" style={{ marginTop: 12 }}>
                                ...
                                <button className="mp-btn mp-ghost-link" type="button" onClick={() => navigate("/shop/panel")} disabled={saving || uploadingImage}>
                                    Cancel
                                </button>
                            </div>
                        </form>
